...
**要在按钮点击后执行某些操作**，你需要在你的插件类中实现`onSettingsButtonClicked(std::string_view)`方法。该方法会在按钮被点击时调用，你可以在其中执行你的操作。例如：
```cpp
...
```
在这个例子中，当用户点击了名为`my_plugin.foo`的按钮时，会弹出一个包含"Hello World!"文字的消息框。
