...
TEST_CASE("makeRational(string) exceptions", "[makeRational][string]") {
    ...
    try { makeRational("a"); } catch (const InvalidRationalParseException<std::string>& e) {
        CHECK(e.origin == "a");
    }
    try { makeRational("1/0"); } catch (const boost::bad_rational&) {
...
TEST_CASE("makeRational(double) exceptions", "[makeRational][double]") {
    ...
    try { makeRational(NAN); } catch (const InvalidRationalParseException<double>& e) {
...
TEST_CASE("pow exceptions", "[pow]") {
    ...
    try { pow(Rational(0), Rational(0)); } catch (const RationalCalculationException& e) {
    ...
    try { pow(Rational(-2), Rational(1,2)); } catch (const RationalCalculationException& e) {
    ...
    try { pow(Rational(0), Rational(-1)); } catch (const RationalCalculationException& e) {
    ...
    try { pow(Rational(2), Rational(1,2), opt); } catch (const RationalCalculationException& e) {
...
TEST_CASE("factorial normal", "[factorial]") {
    CHECK(factorial(cpp_int(0)) == cpp_int(1));
    CHECK(factorial(cpp_int(1)) == cpp_int(1));
    CHECK(factorial(cpp_int(5)) == cpp_int(120));
    CHECK(factorial(cpp_int(10)) == cpp_int(3628800));
}

TEST_CASE("factorial exception", "[factorial]") {
    CHECK_THROWS_AS(factorial(cpp_int(-1)), RationalCalculationException);
    try { factorial(cpp_int(-1)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Pole);
        CHECK(e.operation == RationalCalculationException::Operation::Factorial);
    }
    CHECK_THROWS_AS(factorial(cpp_int(-5)), RationalCalculationException);
    try { factorial(cpp_int(-100)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Pole);
    }
}
...
TEST_CASE("root exception", "[root]") {
    ...
    try { root(Rational(5), Rational(0)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Domain);
    }
    // negative radicand with even index
    CHECK_THROWS_AS(root(Rational(-4), Rational(2)), RationalCalculationException);
    try { root(Rational(-4), Rational(2)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Domain);
    }
}
...
TEST_CASE("sqrt exception", "[sqrt]") {
    ...
    try { sqrt(Rational(-1)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Domain);
    }
}
...
TEST_CASE("sin/cos normal & exceptions", "[trig]") {
    ...
    try { sin(Rational(1), opt); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::IrrationalResult);
...
TEST_CASE("tan/cot/sec/csc poles & irrational", "[trig]") {
    ...
    try { tan(pi_half, opt); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Pole);
    ...
    try { cot(Rational(0), opt); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Pole);
...
TEST_CASE("arcsin/arccos normal & domain", "[invtrig]") {
    ...
    try { arcsin(Rational(2)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Domain);
    }
...
TEST_CASE("arcsec/arccsc domain", "[invtrig]") {
    ...
    try { arcsec(Rational(0)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Domain);
    }
}
...
TEST_CASE("floor/ceil normal", "[rounding]") {
    CHECK(floor(Rational(3,2)) == cpp_int(1));
    CHECK(ceil(Rational(3,2)) == cpp_int(2));
    CHECK(floor(Rational(-3,2)) == cpp_int(-2));
    CHECK(ceil(Rational(-3,2)) == cpp_int(-1));
    CHECK(floor(Rational(4)) == cpp_int(4));
    CHECK(ceil(Rational(4)) == cpp_int(4));
}
...
TEST_CASE("ln exceptions", "[log]") {
    ...
    try { ln(Rational(0)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Pole);
    }
    // negative argument -> Domain
    CHECK_THROWS_AS(ln(Rational(-1)), RationalCalculationException);
    try { ln(Rational(-1)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Domain);
    }
...
TEST_CASE("log exceptions", "[log]") {
    ...
    try { log(Rational(2), Rational(1)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::IndeterminateForm);
    }
...
TEST_CASE("lg exceptions", "[log]") {
    ...
    try { lg(Rational(-1)); } catch (const RationalCalculationException& e) {
        CHECK(e.type == RationalCalculationException::Type::Domain);
    }
    // irrational
...
TEST_CASE("benchmark factorial", "[bench]") {
    BENCHMARK("small") { return factorial(cpp_int(10)); };
    BENCHMARK("medium") { return factorial(cpp_int(50)); };
    BENCHMARK("large") { return factorial(cpp_int(100)); };
}
