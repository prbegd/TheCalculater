...
util::unique_pmr_ptr<AnalyticExpression::Node> AnalyticExpression::Addition::clone(
    std::pmr::memory_resource* memoryResource) const
{
...
std::optional<AnalyticExpression::Simplification::Rule::WildcardMap> AnalyticExpression::Simplification::Rule::
    match(const Node& target, std::pmr::memory_resource* memoryResource) const
{
...
util::unique_pmr_ptr<AnalyticExpression::Node> AnalyticExpression::Simplification::Rule::apply(
    WildcardMap map, std::pmr::memory_resource* memoryResource) const
{
...
namespace { namespace impl::simplification::hill_climbing_algorithm {
        ...
        const AnalyticExpression::Simplification::Context& context, const AnalyticExpression::Node& target)
    {
        ...
        auto candidateNodes =
            ...
            | std::views::transform([&context, &target](const AnalyticExpression::Simplification::Rule& rule) {
                  return std::make_pair(std::cref(rule), rule.match(target, context.memoryResource));
              })
            ...
            | std::views::transform(
                [&context](
                    ...
                    return rulePair.first.apply(std::move(*rulePair.second), context.memoryResource);
                })
            | std::ranges::to<std::pmr::vector<util::unique_pmr_ptr<AnalyticExpression::Node>>>(
    ...
    util::unique_pmr_ptr<AnalyticExpression::Node> applyUntilFixed(
        const AnalyticExpression::Simplification::Context& context, const AnalyticExpression::Node& target)
    {
...
util::unique_pmr_ptr<AnalyticExpression::Node> AnalyticExpression::Simplification::HillClimbingAlgorithm::
operator()(const AnalyticExpression::Simplification::Context& context,
           const AnalyticExpression::Node& target) const
{
...
namespace { namespace impl::simplification::e_graph_algorithm {
    ...
    // just optimization)
    // TODO(P2): implement wildcardMatch separately. instead of returning a boolean, it should generate
    ...
    public:
        ...
        util::unique_pmr_ptr<AnalyticExpression::Node> saturate(
            const AnalyticExpression::Simplification::Context& context, std::uint64_t maxIterations)
        {
    ...
    private:
        ...
        public:
            ...
            std::size_t mergeClass(EClassReference target, EClassReference from)
            {
                ...
                for (const ENode& member : targetClass.members) {
                    ...
                    for (const AnalyticExpression::Node* child : children) {
                        if (typeid(*child) == typeid(EClassReferenceNode)) {
                            self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode*>(child)->reference), processing);
                        }
                    }
                }
        ...
        std::pmr::vector<util::unique_pmr_ptr<AnalyticExpression::Node>> expandAllPossibleSolutions_(
            EClassReference target, std::pmr::memory_resource* memoryResource)
        {
            ...
            const auto expand =
                ...
                for (const ENode& member : targetClass.members) {
                    ...
                    member.node->accept(AnalyticExpression::NodeVisitorConst(
                        [this, &self, &solutions, &processing, &cache, memoryResource](
                            ...
                            for (const util::unique_pmr_ptr<AnalyticExpression::Node>& term : node.terms) {
                                ...
                                termsSolutions.push_back(self(
                                    equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*term).reference), processing, cache));
                            }
                            std::ranges::transform(
                        ...
                        [this, &self, &solutions, &processing, &cache, memoryResource](
                            ...
                            for (const util::unique_pmr_ptr<AnalyticExpression::Node>& factor : node.factors) {
                                ...
                                factorsSolutions.push_back(
                                    self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*factor).reference),
                                         processing,
                                         cache));
                            }
                            std::ranges::transform(
                        ...
                        [this, &self, &solutions, &processing, &cache, memoryResource](
                            ...
                            std::ranges::transform(
                                std::views::cartesian_product(
                                    self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*node.base).reference),
                                         processing,
                                         cache),
                                    self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*node.exponent).reference),
                                         processing,
                                         cache)),
                                std::back_inserter(solutions),
                        ...
                        [this, &self, &solutions, &processing, &cache, memoryResource](
                            ...
                            std::ranges::transform(
                                self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*node.operand).reference),
                                     processing,
                                     cache),
                                std::back_inserter(solutions),
                                [memoryResource](
                        ...
                        [this, &self, &solutions, &processing, &cache, memoryResource](
                            ...
                            std::ranges::transform(
                                std::views::cartesian_product(
                                    self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*node.dividend).reference),
                                         processing,
                                         cache),
                                    self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*node.divisor).reference),
                                         processing,
                                         cache)),
                                std::back_inserter(solutions),
                                [memoryResource](const auto& solution) {
                        ...
                        [this, &self, &solutions, &processing, &cache, memoryResource](
                            ...
                            std::ranges::transform(
                                std::views::cartesian_product(
                                    self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*node.argument).reference),
                                         processing,
                                         cache),
                                    self(equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode&>(*node.base).reference),
                                         processing,
                ...
                | std::views::filter(
                       [this, target](const std::pair<const EClassReference, EClass>& parent) -> bool {
                           ...
                           for (const ENode& member : parent.second.members) {
                               ...
                               if (std::ranges::any_of(children, [this, target](const AnalyticExpression::Node* child) {
                                       return equivalentClassManager_.representativeOf(static_cast<const EClassReferenceNode*>(child)->reference) == target;
                                   })) {
                                   return true;
                               }
                           }
                ...
                auto availableRules =
                    ...
                    | std::views::transform(
                        [&context, &solution](const AnalyticExpression::Simplification::Rule& rule) {
                            return std::make_pair(std::cref(rule), rule.match(*solution, context.memoryResource));
                        })
                    | std::views::cache_latest
                    | std::views::filter(
                        [](const std::pair<const AnalyticExpression::Simplification::Rule&,
...
AnalyticExpression::Simplification::RuleSet AnalyticExpression::Simplification::generateDefaultRules(
    std::pmr::memory_resource* memoryResource)
{
