 ...
 */
export module prbegd.thecalculater.settings:property;
import :value;
...
export struct ItemProperty {
    ...
    [[nodiscard]]
    virtual std::unique_ptr<ItemProperty> clone() const = 0;
};

export using PropertiesType = std::unordered_map<std::string, std::unique_ptr<ItemProperty>, util::TransparentHash<std::string_view>, std::equal_to<>>;
...
export struct NamespaceItemProperty : ItemProperty {
    ...
    {
        return ValueType::Namespace;
    }

    [[nodiscard]]
    std::unique_ptr<ItemProperty> clone() const override
    {
...
export struct NumberItemProperty : ItemProperty {
    ...
    NumberItemProperty(const std::optional<Value>& defaultValue,
                       ...
                       std::optional<double> min = { },
                       std::optional<double> max = { })
        : ItemProperty(defaultValue, name, description, note, warning, deprecated),
          min(min),
          max(max)
    { }
    NumberItemProperty()
