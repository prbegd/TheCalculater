 ...
 */
export module prbegd.thecalculater.settings:value;
...
public:
    ...
    [[nodiscard]]
    thecalculater::math::Rational rational() const
    {
        return value;
    }
    [[nodiscard]]
    ...
    {
        return value.convert_to<std::int8_t>();
    }
    [[nodiscard]]
    std::int16_t int16() const
    {
        return value.convert_to<std::int16_t>();
    }
    [[nodiscard]]
    std::int32_t int32() const
    {
        return value.convert_to<std::int32_t>();
    }
    [[nodiscard]]
    ...
    {
        return value.convert_to<std::uint8_t>();
    }
    [[nodiscard]]
    std::uint16_t uint16() const
    {
        return value.convert_to<std::uint16_t>();
    }
    [[nodiscard]]
...
private:
    thecalculater::math::Rational value;
...
public:
    ...
    template <typename T>
        requires(std::is_floating_point_v<T>)
    explicit DecimalValue(T val)
        : value(thecalculater::math::makeRational(val))
    { }
    DecimalValue() = default;

    static DecimalValue fromString(const std::string& str)
    {
        return DecimalValue(thecalculater::math::makeRational(str));
    }
...
export struct Value : std::variant<BooleanValue, ListValue, ObjectValue, StringValue, IntegerValue, DecimalValue> {
    [[nodiscard]]
    ...
    {
        return std::get<BooleanValue>(*this);
    }
    [[nodiscard]]
    ListValue toList() const
    {
        return std::get<ListValue>(*this);
    }
    [[nodiscard]]
    ObjectValue toObject() const
    {
        return std::get<ObjectValue>(*this);
    }
    [[nodiscard]]
    StringValue toString() const
    {
        return std::get<StringValue>(*this);
    }
    [[nodiscard]]
    IntegerValue toInteger() const
    {
        return std::get<IntegerValue>(*this);
    }
    [[nodiscard]]
    DecimalValue toDecimal() const
    {
        return std::get<DecimalValue>(*this);
    }
