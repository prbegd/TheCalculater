...
namespace ProgramInfo {
    ...
    std::vector<std::string_view> programArgs = {
        "--help", "--version", "--log-level", "-l", "--no-color", "--nocolor"
    };
    ...
    std::string_view helpContent = R"(Usage: TheCalculater [options]
...
Examples:
    TheCalculater -l info
)";
    std::string_view version = "TheCalculater v" PROGRAM_VERSION "by prbegd Posix";
} // namespace ProgramInfo

inline std::tuple<spdlog::level::level_enum> handleArgs(int argc, char* argv[])
{
...
inline void init(int argc, char* argv[])
{
    ...
    auto [logLevel] = handleArgs(argc, argv);
